...
async def getInfo(context):
    await context.send('sussy baka bot v. 0.1.1\ni love you')
...
async def botSay(context, arg):
    ...
    if not path.exists(source):
        await context.send("i can't say " + arg + " yet, baka.")
        return
...
async def genshinRoll(context, arg):
    if arg == "char" or arg == "character" or arg == "c":
        ...
        for r in result:
            ...
            await context.send("Congratulations, degenerate. You got a " + random.choice(responses) + ".")
...
async def stockPrice(context, arg):
    ...
    if data.size == 0: #does stock exist? yfinance returns no data if stock doesn't exist.
        ...
        await context.send("retrieval time: " + str(data.index[0]))
        await context.send("`" + table.get_string() + "`")
